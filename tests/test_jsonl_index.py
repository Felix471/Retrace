...
def write_reference_index(source: Path, output: Path | None = None) -> Path:
    """Write a deterministic test index from the documented physical-line format."""
    records: list[bytes] = []
    offset = 0
    with source.open("rb") as stream:
        for line_no, raw_line in enumerate(stream, start=1):
            status = _reference_status(line_no, raw_line)
    ...
    target.write_bytes(
        HEADER_STRUCT.pack(
            b"RIDX", 1, HEADER_STRUCT.size, stat.st_size, stat.st_mtime_ns, len(records)
        )
        + b"".join(records)
    )
    return target
...
def test_reference_index_parity(tmp_path: Path) -> None:
    source = tmp_path / "records.jsonl"
    source.write_bytes(fixture_bytes())
    ...
    assert [entry.status for entry in loaded.entries] == [
        STATUS_OK,
        STATUS_OK,
        STATUS_OK,
        STATUS_BLANK,
...
@pytest.mark.parametrize(
    "corruption",
    ["truncated", "magic", "version", "count", "offset", "status", "ending"],
)
...
def test_real_binary_parity(tmp_path: Path) -> None:
    ...
    assert list(iter_jsonl_records_indexed(source)) == list(iter_jsonl_records(source))
    reference = write_reference_index(source, tmp_path / "reference.ridx")
    ...
    assert custom.read_bytes() == reference.read_bytes()
