...
index::IndexResult require_build(const std::filesystem::path& source) {
    std::string error;
    const auto result = index::build_index(source, error);
...
TEST(JsonlIndexerTest, EmptyFileProducesHeaderOnlyIndex) {
    ...
    EXPECT_EQ(std::filesystem::file_size(output), 32U);
}
...
TEST(JsonlIndexerTest, UnterminatedFinalLineCountsWithoutExtraTrailingRecord) {
    ...
    const auto first = require_build(unterminated);
    const auto second = require_build(terminated);
...
TEST(JsonlIndexerTest, RoundTripPreservesEveryStatusAndObjectOffsets) {
    ...
    const auto built = require_build(source);
    std::string error;
